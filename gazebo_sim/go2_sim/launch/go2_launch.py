...
def generate_launch_description():
    ...
    This is a simplified launch file for OM1-sim that includes:
    - Gazebo simulation with Go2 robot
    - CHAMP quadruped controller
    - Topic remapping nodes (go2_remapping, go2_sport, go2_lowstate)
    - om_path node for lidar-based obstacle avoidance
    - Teleop support via joystick

    It does NOT include: SLAM, Nav2, intel435_depth, frontier exploration,
    orchestrator, or any navigation stack components.
    """
    ...
    rviz2 = Node(
        ...
        executable="rviz2",
        name="rviz2",
        arguments=["-d", os.path.join(go2_sim, "rviz/rviz.rviz")],
    ...
    # Use spawner nodes to handle the configuration step (load -> configure -> activate)
    ...
    # Go2 sport node (converts Unitree sport API -> cmd_vel)
    go2_sport_node = Node(
        package="go2_sim",
    ...
    # Go2 lowstate node (publishes mock LowState with battery sim)
    ...
    # OM Path node - lidar-based obstacle avoidance path computation
    om_path_node = Node(
        package="om_path",
        executable="om_path",
        name="om_path",
